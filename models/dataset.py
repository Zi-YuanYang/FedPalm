...
class MyDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if int(label) > 400:
            # index = np.random.choice(np.arange(len(self.images_label))[np.array(self.images_label) < 400])
            labels = np.array(self.images_label, dtype=int)  # 确保为整数类型
            valid_indices = np.arange(len(labels))[labels < 400]
            new_index = np.random.choice(valid_indices)  # 随机选择有效索引
        img_path = self.images_path[index]
        label = self.images_label[index]        
        # print(img_path)
        # print(img_path)
...
class MyDataset_general_FL(data.Dataset):
    '''
    Load and process the ROI images::
    ...
    def __init__(self, image_paths, labels, transforms=None, train=True, imside = 128, outchannels = 1):        
